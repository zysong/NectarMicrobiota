...
df.LateArrival<-data.frame()

for (arrival in 0:n.segment) {
  ...
  P[1:(quantile[arrival+1]-season[1]+1)]<-0 #knock out pollinators in the segment
  ...
  result<-data.frame(Arrival=arrival,
  ...
  df.LateArrival<-rbind(df.LateArrival, result)
}

#save the results
write.csv(df.LateArrival, file="PolLateArrival.csv", row.names = FALSE)
...
pdf("PolLateArrival.pdf")
plot(Arrival, PolSum, cex.lab=1.5,  xlab = "Delay of pollinator arrival", ylab = "Pollination")
points(Arrival[Arrival==0 | Arrival==10], predict(lm01), type="l")
dev.off()

pdf("PolDeviationLateArrival.pdf")
boxplot(PolDev~Arrival, data=df.LateArrival, cex.lab=1.5,  xlab = "Delay of pollinator arrival", ylab = "Deviation from proportional pollination loss")
abline(h=0, lty=2)
dev.off()
